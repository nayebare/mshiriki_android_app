...
class MediaRouteVolumeSlider extends AppCompatSeekBar {
    ...
    public MediaRouteVolumeSlider(Context context, AttributeSet attrs) {
        this(context, attrs, C0283R.attr.seekBarStyle);
    }
    ...
    protected void drawableStateChanged() {
        ...
        int alpha = isEnabled() ? MotionEventCompat.ACTION_MASK : (int) (255.0f * this.mDisabledAlpha);
    ...
    public void setColor(int color) {
        if (this.mColor != color) {
            if (Color.alpha(color) != MotionEventCompat.ACTION_MASK) {
