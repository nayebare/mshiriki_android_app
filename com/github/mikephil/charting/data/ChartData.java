...
public abstract class ChartData<T extends DataSet<? extends Entry>> {
    ...
    private void calcXValAverageLength() {
        if (this.mXVals.size() <= 0) {
            this.mXValAverageLength = TextTrackStyle.DEFAULT_FONT_SCALE;
            return;
        }
        float sum = TextTrackStyle.DEFAULT_FONT_SCALE;
    ...
    public void addXValue(String xVal) {
        this.mXValAverageLength = (this.mXValAverageLength + ((float) xVal.length())) / ImageRequest.DEFAULT_IMAGE_BACKOFF_MULT;
