...
public class PieChartRenderer extends DataRenderer {
    ...
    public PieChartRenderer(PieChart chart, ChartAnimator animator, ViewPortHandler viewPortHandler) {
        ...
        this.mTransparentCirclePaint.setAlpha(C0308R.styleable.AppCompatTheme_checkedTextViewStyle);
    ...
    protected void drawDataSet(Canvas c, PieDataSet dataSet) {
        ...
        for (int j = 0; j < entries.size(); j++) {
            ...
            if (((double) Math.abs(e.getVal())) > 1.0E-6d && !this.mChart.needsHighlight(e.getXIndex(), ((PieData) this.mChart.getData()).getIndexOfDataSet(dataSet))) {
                ...
                this.mBitmapCanvas.drawArc(this.mChart.getCircleBox(), ((sliceSpace / ImageRequest.DEFAULT_IMAGE_BACKOFF_MULT) + angle) * this.mAnimator.getPhaseY(), (newangle - (sliceSpace / ImageRequest.DEFAULT_IMAGE_BACKOFF_MULT)) * this.mAnimator.getPhaseY(), true, this.mRenderPaint);
            }
    ...
    public void drawValues(Canvas c) {
        ...
        if (this.mChart.isDrawHoleEnabled()) {
            off = (r - ((r / 100.0f) * this.mChart.getHoleRadius())) / ImageRequest.DEFAULT_IMAGE_BACKOFF_MULT;
        }
        r -= off;
        ...
        for (int i = 0; i < dataSets.size(); i++) {
            ...
            if (dataSet.isDrawValuesEnabled() || drawXVals) {
                ...
                for (int j = 0; j < maxEntry; j++) {
                    ...
                    float offset = drawAngles[cnt] / ImageRequest.DEFAULT_IMAGE_BACKOFF_MULT;
                    ...
                    if (drawXVals && drawYVals) {
                        ...
                        if (!drawXVals && drawYVals) {
                            drawValue(c, formatter, value, entry, 0, x, y + (lineHeight / ImageRequest.DEFAULT_IMAGE_BACKOFF_MULT));
                        }
                    } else if (j < data.getXValCount()) {
                        c.drawText((String) data.getXVals().get(j), x, (lineHeight / ImageRequest.DEFAULT_IMAGE_BACKOFF_MULT) + y, this.mValuePaint);
                    }
                    cnt++;
                }
            }
    ...
    protected void drawCenterText(Canvas c) {
        ...
        if (this.mChart.isDrawCenterTextEnabled() && centerText != null) {
            ...
            if (((double) radiusPercent) > 0.0d) {
                boundingRect.inset((boundingRect.width() - (boundingRect.width() * radiusPercent)) / ImageRequest.DEFAULT_IMAGE_BACKOFF_MULT, (boundingRect.height() - (boundingRect.height() * radiusPercent)) / ImageRequest.DEFAULT_IMAGE_BACKOFF_MULT);
            }
            if (!(centerText.equals(this.mCenterTextLastValue) && boundingRect.equals(this.mCenterTextLastBounds))) {
                ...
                this.mCenterTextLayout = new StaticLayout(centerText, 0, centerText.length(), this.mCenterTextPaint, (int) Math.max(Math.ceil((double) this.mCenterTextLastBounds.width()), 1.0d), Alignment.ALIGN_CENTER, TextTrackStyle.DEFAULT_FONT_SCALE, 0.0f, false);
            }
            float layoutHeight = (float) this.mCenterTextLayout.getHeight();
            c.save();
            c.translate(boundingRect.left, boundingRect.top + ((boundingRect.height() - layoutHeight) / ImageRequest.DEFAULT_IMAGE_BACKOFF_MULT));
        ...
        for (int i = 0; i < indices.length; i++) {
            ...
            if (xIndex < drawAngles.length) {
                ...
                if (set != null && set.isHighlightEnabled()) {
                    ...
                    this.mBitmapCanvas.drawArc(highlighted, (set.getSliceSpace() / ImageRequest.DEFAULT_IMAGE_BACKOFF_MULT) + angle, (this.mAnimator.getPhaseY() * sliceDegrees) - (set.getSliceSpace() / ImageRequest.DEFAULT_IMAGE_BACKOFF_MULT), true, this.mRenderPaint);
                }
            }
        }
    }
    ...
    protected void drawRoundedSlices(Canvas c) {
        if (this.mChart.isDrawRoundedSlicesEnabled()) {
            ...
            if (dataSet.isVisible()) {
                PointF center = this.mChart.getCenterCircleBox();
                float r = this.mChart.getRadius();
                float circleRadius = (r - ((this.mChart.getHoleRadius() * r) / 100.0f)) / ImageRequest.DEFAULT_IMAGE_BACKOFF_MULT;
                List<Entry> entries = dataSet.getYVals();
                float[] drawAngles = this.mChart.getDrawAngles();
                float angle = this.mChart.getRotationAngle();
                for (int j = 0; j < entries.size(); j++) {
                    float newangle = drawAngles[j];
                    if (((double) Math.abs(((Entry) entries.get(j)).getVal())) > 1.0E-6d) {
                        double phaseY = (double) ((angle + newangle) * this.mAnimator.getPhaseY());
                        float x = (float) ((((double) (r - circleRadius)) * Math.cos(Math.toRadians(r0))) + ((double) center.x));
                        phaseY = (double) ((angle + newangle) * this.mAnimator.getPhaseY());
                        float y = (float) ((((double) (r - circleRadius)) * Math.sin(Math.toRadians(r0))) + ((double) center.y));
                        this.mRenderPaint.setColor(dataSet.getColor(j));
