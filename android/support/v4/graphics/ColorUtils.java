...
public final class ColorUtils {
    ...
    public static void RGBToHSL(@IntRange(from = 0, to = 255) int r, @IntRange(from = 0, to = 255) int g, @IntRange(from = 0, to = 255) int b, @NonNull float[] outHsl) {
        ...
        if (max == min) {
            ...
            s = deltaMaxMin / (TextTrackStyle.DEFAULT_FONT_SCALE - Math.abs((ImageRequest.DEFAULT_IMAGE_BACKOFF_MULT * l) - TextTrackStyle.DEFAULT_FONT_SCALE));
        }
        h = (60.0f * h) % 360.0f;
        if (h < 0.0f) {
            h += 360.0f;
        ...
        outHsl[MIN_ALPHA_SEARCH_PRECISION] = constrain(s, 0.0f, (float) TextTrackStyle.DEFAULT_FONT_SCALE);
        outHsl[2] = constrain(l, 0.0f, (float) TextTrackStyle.DEFAULT_FONT_SCALE);
    }
    ...
    public static int HSLToColor(@NonNull float[] hsl) {
        ...
        float l = hsl[2];
        float c = (TextTrackStyle.DEFAULT_FONT_SCALE - Math.abs((ImageRequest.DEFAULT_IMAGE_BACKOFF_MULT * l) - TextTrackStyle.DEFAULT_FONT_SCALE)) * s;
        float m = l - (0.5f * c);
        float x = c * (TextTrackStyle.DEFAULT_FONT_SCALE - Math.abs(((h / 60.0f) % ImageRequest.DEFAULT_IMAGE_BACKOFF_MULT) - TextTrackStyle.DEFAULT_FONT_SCALE));
        int r = 0;
        int g = 0;
        int b = 0;
        switch (((int) h) / 60) {
