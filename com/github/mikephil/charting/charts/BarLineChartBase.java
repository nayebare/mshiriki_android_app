...
public abstract class BarLineChartBase<T extends BarLineScatterCandleBubbleData<? extends BarLineScatterCandleBubbleDataSet<? extends Entry>>> extends Chart<T> implements BarLineScatterCandleBubbleDataProvider {
    ...
    class C04641 implements Runnable {
        ...
        C04641(float f, float f2, float f3, float f4) {
            this.val$left = f;
            this.val$top = f2;
            this.val$right = f3;
            this.val$bottom = f4;
    ...
    protected void init() {
        ...
        this.mBorderPaint.setStrokeWidth(Utils.convertDpToPixel(TextTrackStyle.DEFAULT_FONT_SCALE));
    }
    ...
    protected void calcMinMax() {
        ...
        if (leftRange == 0.0f) {
            maxLeft += TextTrackStyle.DEFAULT_FONT_SCALE;
            if (!this.mAxisLeft.isStartAtZeroEnabled()) {
                minLeft -= TextTrackStyle.DEFAULT_FONT_SCALE;
            }
        }
        if (rightRange == 0.0f) {
            maxRight += TextTrackStyle.DEFAULT_FONT_SCALE;
            if (!this.mAxisRight.isStartAtZeroEnabled()) {
                minRight -= TextTrackStyle.DEFAULT_FONT_SCALE;
            }
        }
        ...
        if (!this.mCustomViewPortEnabled) {
            ...
            if (this.mLegend != null && this.mLegend.isEnabled()) {
                if (this.mLegend.getPosition() == LegendPosition.RIGHT_OF_CHART || this.mLegend.getPosition() == LegendPosition.RIGHT_OF_CHART_CENTER) {
                    offsetRight = 0.0f + (Math.min(this.mLegend.mNeededWidth, this.mViewPortHandler.getChartWidth() * this.mLegend.getMaxSizePercent()) + (this.mLegend.getXOffset() * ImageRequest.DEFAULT_IMAGE_BACKOFF_MULT));
                    ...
                    offsetLeft = 0.0f + (Math.min(this.mLegend.mNeededWidth, this.mViewPortHandler.getChartWidth() * this.mLegend.getMaxSizePercent()) + (this.mLegend.getXOffset() * ImageRequest.DEFAULT_IMAGE_BACKOFF_MULT));
    ...
    protected float[] getMarkerPosition(Entry e, Highlight highlight) {
        ...
        if (this instanceof BarChart) {
            ...
            if (this instanceof HorizontalBarChart) {
                yPos = (((float) ((((setCount - 1) * i) + i) + dataSetIndex)) + (((float) i) * space)) + (space / ImageRequest.DEFAULT_IMAGE_BACKOFF_MULT);
                ...
                xPos = (((float) ((((setCount - 1) * i) + i) + dataSetIndex)) + (((float) i) * space)) + (space / ImageRequest.DEFAULT_IMAGE_BACKOFF_MULT);
                if (((BarEntry) e).getVals() != null) {
    ...
    public void zoomIn() {
        this.mViewPortHandler.refresh(this.mViewPortHandler.zoomIn(((float) getWidth()) / ImageRequest.DEFAULT_IMAGE_BACKOFF_MULT, -(((float) getHeight()) / ImageRequest.DEFAULT_IMAGE_BACKOFF_MULT)), this, true);
        calculateOffsets();
        postInvalidate();
    }

    public void zoomOut() {
        this.mViewPortHandler.refresh(this.mViewPortHandler.zoomOut(((float) getWidth()) / ImageRequest.DEFAULT_IMAGE_BACKOFF_MULT, -(((float) getHeight()) / ImageRequest.DEFAULT_IMAGE_BACKOFF_MULT)), this, true);
        calculateOffsets();
        postInvalidate();
    }
    ...
    public void moveViewToY(float yValue, AxisDependency axis) {
        Runnable job = new MoveViewJob(this.mViewPortHandler, 0.0f, ((getDeltaY(axis) / this.mViewPortHandler.getScaleY()) / ImageRequest.DEFAULT_IMAGE_BACKOFF_MULT) + yValue, getTransformer(axis), this);
        if (this.mViewPortHandler.hasChartDimens()) {
            post(job);
        } else {
            this.mJobs.add(job);
    ...
    public void moveViewTo(float xIndex, float yValue, AxisDependency axis) {
        float f = xIndex;
        Runnable job = new MoveViewJob(this.mViewPortHandler, f, yValue + ((getDeltaY(axis) / this.mViewPortHandler.getScaleY()) / ImageRequest.DEFAULT_IMAGE_BACKOFF_MULT), getTransformer(axis), this);
        if (this.mViewPortHandler.hasChartDimens()) {
            post(job);
        } else {
            this.mJobs.add(job);
    ...
    public void centerViewTo(int xIndex, float yValue, AxisDependency axis) {
        ...
        Runnable job = new MoveViewJob(this.mViewPortHandler, ((float) xIndex) - (xsInView / ImageRequest.DEFAULT_IMAGE_BACKOFF_MULT), ((getDeltaY(axis) / this.mViewPortHandler.getScaleY()) / ImageRequest.DEFAULT_IMAGE_BACKOFF_MULT) + yValue, getTransformer(axis), this);
        if (this.mViewPortHandler.hasChartDimens()) {
            post(job);
        } else {
            this.mJobs.add(job);
    ...
    public void setViewPortOffsets(float left, float top, float right, float bottom) {
        ...
        post(new C04641(left, top, right, bottom));
    }
    ...
    public float getYValueByTouchPoint(float x, float y, AxisDependency axis) {
        return (float) getValuesByTouchPoint(x, y, axis).f21y;
    }
    ...
    public int getLowestVisibleXIndex() {
        ...
        return (int) (pts[0] + TextTrackStyle.DEFAULT_FONT_SCALE);
    }
