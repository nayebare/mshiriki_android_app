...
class zzaza implements zzays {
    ...
    private static final String zzbHX;
    private static final String zzbHY;
    private static final String zzbHZ;
    ...
    class zza implements com.google.android.gms.internal.zzazl.zza {
        ...
        public void zzb(zzayo com_google_android_gms_internal_zzayo) {
            this.zzbId.zzu(com_google_android_gms_internal_zzayo.zzPi());
            zzayx.m24v("Permanent failure dispatching hitId: " + com_google_android_gms_internal_zzayo.zzPi());
        }
        ...
        public void zzc(zzayo com_google_android_gms_internal_zzayo) {
            ...
            if (zzPj == 0) {
                ...
                this.zzbId.zzu(com_google_android_gms_internal_zzayo.zzPi());
                zzayx.m24v("Giving up on failed hitId: " + com_google_android_gms_internal_zzayo.zzPi());
            }
        }
    }
    ...
    static {
        zzbHX = String.format("CREATE TABLE IF NOT EXISTS %s ('%s' TEXT UNIQUE);", new Object[]{"gtm_hit_unique_ids", "hit_unique_id"});
        ...
        zzbHY = String.format("CREATE TRIGGER IF NOT EXISTS %s DELETE ON %s FOR EACH ROW WHEN OLD.%s NOTNULL BEGIN     INSERT OR IGNORE INTO %s (%s) VALUES (OLD.%s); END;", new Object[]{"save_unique_on_delete", "gtm_hits", "hit_unique_id", "gtm_hit_unique_ids", "hit_unique_id", "hit_unique_id"});
        zzbHZ = String.format("CREATE TRIGGER IF NOT EXISTS %s BEFORE INSERT ON %s FOR EACH ROW WHEN NEW.%s NOT NULL BEGIN     SELECT RAISE(ABORT, 'Duplicate unique ID.')     WHERE EXISTS (SELECT 1 FROM %s WHERE %s = NEW.%s); END;", new Object[]{"check_unique_on_insert", "gtm_hits", "hit_unique_id", "gtm_hit_unique_ids", "hit_unique_id", "hit_unique_id"});
    }

    zzaza(zzayt com_google_android_gms_internal_zzayt, Context context) {
        this(com_google_android_gms_internal_zzayt, context, "gtm_urls.db", GamesStatusCodes.STATUS_REQUEST_UPDATE_PARTIAL_SUCCESS);
    }
    ...
    private void zzPv() {
        ...
        if (zzPw > 0) {
            List zzmV = zzmV(zzPw);
            zzayx.m24v("Store full, deleting " + zzmV.size() + " hits to make room.");
    ...
    private void zzb(long j, String str, @Nullable String str2, @Nullable String str3, @Nullable Map<String, String> map, @Nullable String str4) {
        ...
        if (zzhh != null) {
            ...
            try {
                ...
                zzayx.m24v(new StringBuilder(String.valueOf(str).length() + 19).append("Hit stored (url = ").append(str).append(")").toString());
                this.zzbIc.zzaM(false);
            } catch (SQLiteConstraintException e) {
                String str7 = "Hit has already been sent: ";
                str5 = String.valueOf(str);
                zzayx.m24v(str5.length() != 0 ? str7.concat(str5) : new String(str7));
            } catch (SQLiteException e2) {
    ...
    public void dispatch() {
        zzayx.m24v("GTM Dispatch running...");
        if (this.zzbIb.zzPa()) {
            List zzmW = zzmW(40);
            if (zzmW.isEmpty()) {
                zzayx.m24v("...nothing to dispatch");
    ...
    public void zza(long j, String str, @Nullable String str2, @Nullable String str3, @Nullable Map<String, String> map, @Nullable String str4) {
        ...
        if (zzazc.zzQB().isPreview()) {
            zzayx.m24v("Sending hits immediately under preview.");
            dispatch();
        }
    }
    ...
    int zznS() {
        ...
        zzayx.m24v("Removed " + delete + " stale hits.");
        zzayt com_google_android_gms_internal_zzayt = this.zzbIc;
        if (zzPw() != 0) {
            z = false;
        }
