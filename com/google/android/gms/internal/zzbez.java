...
public class zzbez extends zzazv {
    ...
    private static final List<String> zzbGY;
    private static final Pattern zzbGZ;
    private static final Pattern zzbHa;
    private static final Set<String> zzbJN;
    private static final Map<String, String> zzbJO;
    private static final Map<String, String> zzbJP;
    ...
    static {
        ...
        zzbGY = Arrays.asList(new String[]{ProductAction.ACTION_DETAIL, ProductAction.ACTION_CHECKOUT, ProductAction.ACTION_CHECKOUT_OPTION, Promotion.ACTION_CLICK, ProductAction.ACTION_ADD, ProductAction.ACTION_REMOVE, ProductAction.ACTION_PURCHASE, ProductAction.ACTION_REFUND});
        zzbGZ = Pattern.compile("dimension(\\d+)");
        zzbHa = Pattern.compile("metric(\\d+)");
        zzbJN = zzf.zza(BuildConfig.FLAVOR, "0", "false");
        zzbJO = zzf.zza("transactionId", "&ti", "transactionAffiliation", "&ta", "transactionTax", "&tt", "transactionShipping", "&ts", "transactionTotal", "&tr", "transactionCurrency", "&cu");
        zzbJP = zzf.zza("name", "&in", "sku", "&ic", "category", "&iv", "price", "&ip", "quantity", "&iq", "currency", "&cu");
    }
    ...
    private void zza(Tracker tracker, zzbfw<?> com_google_android_gms_internal_zzbfw_, zzbfw<?> com_google_android_gms_internal_zzbfw_2, zzbfw<?> com_google_android_gms_internal_zzbfw_3) {
        ...
        if (str == null) {
            zzayx.m23e("Cannot find transactionId in data layer.");
            return;
        }
            ...
            if (zzhS != null) {
                for (Map map : zzhS) {
                    if (map.get("name") == null) {
                        zzayx.m23e("Unable to send transaction item hit due to missing 'name' field.");
                        return;
                    }
    ...
    private Product zzaq(Map<String, Object> map) {
        ...
        for (String valueOf2 : map.keySet()) {
            Matcher matcher = zzbGZ.matcher(valueOf2);
            if (matcher.matches()) {
                try {
                    ...
                    zzayx.zzbe(valueOf2.length() != 0 ? str.concat(valueOf2) : new String(str));
                }
            } else {
                matcher = zzbHa.matcher(valueOf2);
                if (matcher.matches()) {
                    try {
                        product.setCustomMetric(Integer.parseInt(matcher.group(1)), zzW(map.get(valueOf2)).intValue());
            ...
            if (obj instanceof List) {
                for (Map zzi2 : (List) obj) {
                    try {
                        ...
                        zzayx.m23e(str2.length() != 0 ? str3.concat(str2) : new String(str3));
                    }
                }
            }
            List list2 = null;
            ...
            if (r1 != null) {
                for (Map zzi22 : r1) {
                    try {
                        ...
                        zzayx.m23e(str2.length() != 0 ? str.concat(str2) : new String(str));
                    }
                }
                if (map.containsKey("promoClick")) {
                        ...
                        for (String str22 : zzbGY) {
                            if (map.containsKey(str22)) {
                                ...
                                try {
                                    ...
                                    zzayx.m23e(valueOf2.length() != 0 ? str22.concat(valueOf2) : new String(str22));
                                }
                            }
                        }
                    }
