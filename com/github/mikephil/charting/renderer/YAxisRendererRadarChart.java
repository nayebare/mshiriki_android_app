...
public class YAxisRendererRadarChart extends YAxisRenderer {
    ...
    protected void computeAxisValues(float min, float max) {
        ...
        if (this.mYAxis.isForceLabelsEnabled()) {
            ...
            length = this.mYAxis.mEntries.length;
            if (r0 < labelCount) {
                fArr = new float[labelCount];
                this.mYAxis.mEntries = fArr;
            }
            float v = min;
        ...
        if (interval < 1.0d) {
            yAxis = this.mYAxis;
            yAxis.mDecimals = (int) Math.ceil(-Math.log10(interval));
        } else {
        ...
        if (!this.mYAxis.isStartAtZeroEnabled()) {
            if (this.mYAxis.mEntries[0] < yMin) {
                yAxis = this.mYAxis;
                yAxis.mAxisMinimum = this.mYAxis.mEntries[0];
            }
        }
        yAxis = this.mYAxis;
        yAxis.mAxisMaximum = this.mYAxis.mEntries[this.mYAxis.mEntryCount - 1];
        yAxis = this.mYAxis;
        yAxis.mAxisRange = Math.abs(this.mYAxis.mAxisMaximum - this.mYAxis.mAxisMinimum);
    }
