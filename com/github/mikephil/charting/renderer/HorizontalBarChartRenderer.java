...
public class HorizontalBarChartRenderer extends BarChartRenderer {
    ...
    public void drawValues(Canvas c) {
        if (passesCheck()) {
            ...
            int i = 0;
            while (true) {
                if (i < this.mChart.getBarData().getDataSetCount()) {
                    ...
                    if (dataSet.isDrawValuesEnabled() && dataSet.getEntryCount() != 0) {
                        ...
                        float halfTextHeight = ((float) Utils.calcTextHeight(this.mValuePaint, "10")) / ImageRequest.DEFAULT_IMAGE_BACKOFF_MULT;
                        ...
                        if (!dataSet.isStacked()) {
                            j = 0;
                            while (true) {
                                if (((float) j) >= ((float) valuePoints.length) * this.mAnimator.getPhaseX()) {
                                    break;
                                }
                            ...
                            j = 0;
                            while (true) {
                                if (((float) j) >= ((float) (valuePoints.length - 1)) * this.mAnimator.getPhaseX()) {
                                    break;
                                }
                                e = (BarEntry) entries.get(j / 2);
