...
public abstract class Utils {
    ...
    static {
        ...
        mMaximumFlingVelocity = GamesStatusCodes.STATUS_MILESTONE_CLAIMED_PREVIOUSLY;
        POW_10 = new int[]{1, 10, 100, CardNetwork.OTHER, 10000, 100000, 1000000, 10000000, 100000000, 1000000000};
    ...
    public static String formatNumber(float number, int digitCount, boolean separateThousands, char separateChar) {
        ...
        boolean zero = false;
        if (number < TextTrackStyle.DEFAULT_FONT_SCALE && number > PlayerStats.UNSET_VALUE) {
            zero = true;
        }
        if (number < 0.0f) {
            neg = true;
    ...
    public static double nextUp(double d) {
        if (d == MediaQueueItem.DEFAULT_PLAYBACK_DURATION) {
            return d;
        }
        d += 0.0d;
    ...
    public static void velocityTrackerPointerUpCleanUpIfNecessary(MotionEvent ev, VelocityTracker tracker) {
        tracker.computeCurrentVelocity(CardNetwork.OTHER, (float) mMaximumFlingVelocity);
