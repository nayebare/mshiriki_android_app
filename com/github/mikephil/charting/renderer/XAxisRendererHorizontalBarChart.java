...
public class XAxisRendererHorizontalBarChart extends XAxisRendererBarChart {
    ...
    public void renderAxisLabels(Canvas c) {
        if (this.mXAxis.isEnabled() && this.mXAxis.isDrawLabelsEnabled()) {
            ...
            if (this.mXAxis.getPosition() == XAxisPosition.TOP) {
                ...
                drawLabels(c, this.mViewPortHandler.contentRight() - xoffset, new PointF(TextTrackStyle.DEFAULT_FONT_SCALE, 0.5f));
            } else if (this.mXAxis.getPosition() == XAxisPosition.BOTTOM) {
                drawLabels(c, this.mViewPortHandler.contentLeft() - xoffset, new PointF(TextTrackStyle.DEFAULT_FONT_SCALE, 0.5f));
    ...
    protected void drawLabels(Canvas c, float pos, PointF anchor) {
        ...
        int i = this.mMinX;
        while (i <= this.mMaxX) {
            position[1] = (((float) (i * step)) + (((float) i) * bd.getGroupSpace())) + (bd.getGroupSpace() / ImageRequest.DEFAULT_IMAGE_BACKOFF_MULT);
            if (step > 1) {
                position[1] = position[1] + ((((float) step) - TextTrackStyle.DEFAULT_FONT_SCALE) / ImageRequest.DEFAULT_IMAGE_BACKOFF_MULT);
            }
