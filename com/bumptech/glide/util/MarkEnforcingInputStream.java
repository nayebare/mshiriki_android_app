...
public class MarkEnforcingInputStream extends FilterInputStream {
    ...
    public int available() throws IOException {
        return this.availableBytes == UNSET ? super.available() : Math.min(this.availableBytes, super.available());
    }
    ...
    private long getBytesToRead(long targetByteCount) {
        ...
        if (this.availableBytes == UNSET || targetByteCount <= ((long) this.availableBytes)) {
    ...
    private void updateAvailableBytesAfterRead(long bytesRead) {
        if (this.availableBytes != UNSET && bytesRead != -1) {
