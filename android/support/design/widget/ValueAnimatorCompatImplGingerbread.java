...
class ValueAnimatorCompatImplGingerbread extends Impl {
    ...
    public void end() {
        if (this.mIsRunning) {
            this.mIsRunning = false;
            sHandler.removeCallbacks(this.mRunnable);
            this.mAnimatedFraction = TextTrackStyle.DEFAULT_FONT_SCALE;
            dispatchAnimationUpdate();
            dispatchAnimationEnd();
        }
    }
    ...
    final void update() {
        if (this.mIsRunning) {
            float linearFraction = MathUtils.constrain(((float) (SystemClock.uptimeMillis() - this.mStartTime)) / ((float) this.mDuration), 0.0f, (float) TextTrackStyle.DEFAULT_FONT_SCALE);
            if (this.mInterpolator != null) {
