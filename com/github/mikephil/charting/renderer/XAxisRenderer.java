    ...
    public void renderAxisLabels(Canvas c) {
        if (this.mXAxis.isEnabled() && this.mXAxis.isDrawLabelsEnabled()) {
            ...
            if (this.mXAxis.getPosition() == XAxisPosition.TOP) {
                drawLabels(c, this.mViewPortHandler.contentTop() - yoffset, new PointF(0.5f, TextTrackStyle.DEFAULT_FONT_SCALE));
            } else if (this.mXAxis.getPosition() == XAxisPosition.TOP_INSIDE) {
                drawLabels(c, (this.mViewPortHandler.contentTop() + yoffset) + ((float) this.mXAxis.mLabelRotatedHeight), new PointF(0.5f, TextTrackStyle.DEFAULT_FONT_SCALE));
            ...
            if (this.mViewPortHandler.isInBoundsX(position[0])) {
                ...
                if (this.mXAxis.isAvoidFirstLastClippingEnabled()) {
                    if (i == this.mXAxis.getValues().size() - 1 && this.mXAxis.getValues().size() > 1) {
                        ...
                        if (width > this.mViewPortHandler.offsetRight() * ImageRequest.DEFAULT_IMAGE_BACKOFF_MULT && position[0] + width > this.mViewPortHandler.getChartWidth()) {
                            position[0] = position[0] - (width / ImageRequest.DEFAULT_IMAGE_BACKOFF_MULT);
                        }
                    } else if (i == 0) {
                        position[0] = position[0] + (((float) Utils.calcTextWidth(this.mAxisLabelPaint, label)) / ImageRequest.DEFAULT_IMAGE_BACKOFF_MULT);
                    }
                }
        ...
        if (limitLines != null && limitLines.size() > 0) {
            ...
            for (int i = 0; i < limitLines.size(); i++) {
                ...
                if (l.isEnabled()) {
                    ...
                    renderLimitLineLabel(c, l, position, ImageRequest.DEFAULT_IMAGE_BACKOFF_MULT + l.getYOffset());
                }
            }
        }
    }
