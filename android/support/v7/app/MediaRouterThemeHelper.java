...
final class MediaRouterThemeHelper {
    ...
    public static Context createThemedContext(Context context, int style) {
        ...
        if (isLightTheme(context)) {
            if (getControllerColor(context, style) == COLOR_DARK_ON_LIGHT_BACKGROUND) {
                theme = C0308R.style.Theme_MediaRouter_Light;
            } else {
                theme = C0308R.style.Theme_MediaRouter_Light_DarkControlPanel;
            }
        } else if (getControllerColor(context, style) == COLOR_DARK_ON_LIGHT_BACKGROUND) {
            theme = C0308R.style.Theme_MediaRouter_LightControlPanel;
        } else {
            theme = C0308R.style.Theme_MediaRouter;
        }
    ...
    public static int getControllerColor(Context context, int style) {
        if (ColorUtils.calculateContrast(COLOR_WHITE_ON_DARK_BACKGROUND, getThemeColor(context, style, C0283R.attr.colorPrimary)) >= 3.0d) {
    ...
    public static int getButtonTextColor(Context context) {
        int primaryColor = getThemeColor(context, 0, C0283R.attr.colorPrimary);
        if (ColorUtils.calculateContrast(primaryColor, getThemeColor(context, 0, 16842801)) < 3.0d) {
            return getThemeColor(context, 0, C0283R.attr.colorAccent);
        }
        return primaryColor;
    }
    ...
    public static void setMediaControlsBackgroundColor(Context context, View mainControls, View groupControls, boolean hasGroup) {
        int primaryColor = getThemeColor(context, 0, C0283R.attr.colorPrimary);
        int primaryDarkColor = getThemeColor(context, 0, C0283R.attr.colorPrimaryDark);
        if (hasGroup && getControllerColor(context, 0) == COLOR_DARK_ON_LIGHT_BACKGROUND) {
            primaryDarkColor = primaryColor;
            primaryColor = COLOR_WHITE_ON_DARK_BACKGROUND;
        }
    ...
    public static void setVolumeSliderColor(Context context, MediaRouteVolumeSlider volumeSlider, View backgroundView) {
        ...
        if (Color.alpha(controllerColor) != MotionEventCompat.ACTION_MASK) {
    ...
    private static boolean isLightTheme(Context context) {
        TypedValue value = new TypedValue();
        if (!context.getTheme().resolveAttribute(C0283R.attr.isLightTheme, value, true) || value.data == 0) {
            return false;
        }
        return true;
    }
