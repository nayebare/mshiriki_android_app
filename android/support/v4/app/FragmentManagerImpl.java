...
*/
    ...
    Animation loadAnimation(Fragment fragment, int transit, boolean enter, int transitionStyle) {
        ...
        switch (styleIndex) {
            case ANIM_STYLE_OPEN_ENTER /*1*/:
                return makeOpenCloseAnimation(this.mHost.getContext(), 1.125f, TextTrackStyle.DEFAULT_FONT_SCALE, 0.0f, TextTrackStyle.DEFAULT_FONT_SCALE);
            case ANIM_STYLE_OPEN_EXIT /*2*/:
                return makeOpenCloseAnimation(this.mHost.getContext(), TextTrackStyle.DEFAULT_FONT_SCALE, 0.975f, TextTrackStyle.DEFAULT_FONT_SCALE, 0.0f);
            case ANIM_STYLE_CLOSE_ENTER /*3*/:
                return makeOpenCloseAnimation(this.mHost.getContext(), 0.975f, TextTrackStyle.DEFAULT_FONT_SCALE, 0.0f, TextTrackStyle.DEFAULT_FONT_SCALE);
            case ANIM_STYLE_CLOSE_EXIT /*4*/:
                return makeOpenCloseAnimation(this.mHost.getContext(), TextTrackStyle.DEFAULT_FONT_SCALE, 1.075f, TextTrackStyle.DEFAULT_FONT_SCALE, 0.0f);
            case ANIM_STYLE_FADE_ENTER /*5*/:
                return makeFadeAnimation(this.mHost.getContext(), 0.0f, TextTrackStyle.DEFAULT_FONT_SCALE);
            case ANIM_STYLE_FADE_EXIT /*6*/:
                return makeFadeAnimation(this.mHost.getContext(), TextTrackStyle.DEFAULT_FONT_SCALE, 0.0f);
            default:
    ...
    private int postponePostponableTransactions(ArrayList<BackStackRecord> records, ArrayList<Boolean> isRecordPop, int startIndex, int endIndex, ArraySet<Fragment> added) {
        ...
        while (i >= startIndex) {
            ...
            if (!record.isPostponed() || record.interactsWith(records, i + ANIM_STYLE_OPEN_ENTER, endIndex)) {
                isPostponed = HONEYCOMB;
            } else {
                isPostponed = true;
            }
            if (isPostponed) {
                ...
                if (isPop) {
                    ...
                    record.executePopOps(HONEYCOMB);
                }
                postponeIndex--;
                if (i != postponeIndex) {
                    records.remove(i);
            ...
            for (int i = 0; i < numActive; i += ANIM_STYLE_OPEN_ENTER) {
                ...
                if (fragment != null && fragment.mView != null && fragment.mIsNewlyAdded && record.interactsWith(fragment.mContainerId)) {
                    ...
                    if (moveToState) {
                        ...
                        fragment.mPostponedAlpha = PlayerStats.UNSET_VALUE;
    ...
    private static void executeOps(ArrayList<BackStackRecord> records, ArrayList<Boolean> isRecordPop, int startIndex, int endIndex) {
        ...
        while (i < endIndex) {
            ...
            if (((Boolean) isRecordPop.get(i)).booleanValue()) {
                ...
                record.executePopOps(i == endIndex + -1 ? true : HONEYCOMB);
            } else {
                record.bumpBackStackNesting(ANIM_STYLE_OPEN_ENTER);
                record.executeOps();
            }
            i += ANIM_STYLE_OPEN_ENTER;
        }
