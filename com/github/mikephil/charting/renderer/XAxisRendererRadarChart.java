    ...
    public void renderAxisLabels(Canvas c) {
        if (this.mXAxis.isEnabled() && this.mXAxis.isDrawLabelsEnabled()) {
            ...
            for (int i = 0; i < this.mXAxis.getValues().size(); i += mod) {
                ...
                PointF p = Utils.getPosition(center, (this.mChart.getYRange() * factor) + (((float) this.mXAxis.mLabelRotatedWidth) / ImageRequest.DEFAULT_IMAGE_BACKOFF_MULT), ((((float) i) * sliceangle) + this.mChart.getRotationAngle()) % 360.0f);
                drawLabel(c, label, i, p.x, p.y - (((float) this.mXAxis.mLabelRotatedHeight) / ImageRequest.DEFAULT_IMAGE_BACKOFF_MULT), drawLabelAnchor, labelRotationAngleDegrees);
            }
        }
    }
