...
public class Transformer {
    ...
    public void prepareMatrixOffset(boolean inverted) {
        ...
        if (inverted) {
            ...
            this.mMatrixOffset.postScale(TextTrackStyle.DEFAULT_FONT_SCALE, PlayerStats.UNSET_VALUE);
            return;
        }
    ...
    public float[] generateTransformedValuesBarChart(List<? extends Entry> entries, int dataSet, BarData bd, float phaseY) {
        ...
        for (int j = 0; j < valuePoints.length; j += 2) {
            Entry e = (Entry) entries.get(j / 2);
            int i = e.getXIndex();
            float x = (((float) ((e.getXIndex() + ((setCount - 1) * i)) + dataSet)) + (((float) i) * space)) + (space / ImageRequest.DEFAULT_IMAGE_BACKOFF_MULT);
            float y = e.getVal();
            valuePoints[j] = x;
    ...
    public float[] generateTransformedValuesHorizontalBarChart(List<? extends Entry> entries, int dataSet, BarData bd, float phaseY) {
        ...
        for (int j = 0; j < valuePoints.length; j += 2) {
            Entry e = (Entry) entries.get(j / 2);
            int i = e.getXIndex();
            float x = (((float) ((((setCount - 1) * i) + i) + dataSet)) + (((float) i) * space)) + (space / ImageRequest.DEFAULT_IMAGE_BACKOFF_MULT);
